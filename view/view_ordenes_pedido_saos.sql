create or replace force view "PEVISA"."VIEW_ORDENES_PEDIDO_SAOS"
      ("COD_CLIENTE", "NOMBRE", "FCH_PEDIDO", "PEDIDO", "PEDIDO_ITEM", "NUMERO", "NUOT_SERIE",
       "NUOT_TIPOOT_CODIGO", "FECHA", "JABA", "ESTADO", "ART_COD_ART", "CANT_FORMULA",
       "RENDIMIENTO", "SALDO", "DESPACHAR", "COD_LIN", "FORMU_ART_COD_ART", "ABRE02", "PREUNI",
       "PAIS", "VENDEDOR", "EMPAQUE", "VALOR", "STOCK", "TIENE_STOCK", "ES_SAO", "CANT_PROG",
       "TIENE_IMPORTADO", "TIENE_STOCK_OT", "TIENE_STOCK_ITEM", "IMPRESO", "FCH_IMPRESION",
       "ES_JUEGO", "ES_IMPORTADO", "ES_PRIORITARIO", "ORDEN_PRIORITARIO", "ES_SIMULACION",
       "ES_RESERVADO", "ES_NUEVO", "TOTAL_ART")
as
    ...
    select p.numero as pedido, d.nro as pedido_item, p.cod_cliente, p.nombre, p.fecha as fch_pedido
      ...
      from expednac p
           ...
           right join ordenes o
                      on o.pedido = p.numero
                        and o.pedido_item = d.nro
                        and o.destino = '2' -- JOIN prioridad_marcada m ON p.numero = m.id_pedido
    )
     , detalle
    as (
...
--                                    AS es_nuevo
      from pedidos p
           ...
           left join grupo_cliente gc on gc.cod_grupo = gcc.cod_grupo -- LEFT JOIN cliente_nuevo n
...
select d.cod_cliente, d.nombre, d.fch_pedido, d.pedido, d.pedido_item, d.numero, d.nuot_serie
     ...
     , d.empaque, d.valor, d.stock, d.tiene_stock, 1 as es_sao, d.cant_prog, case max(
    d.es_importado)
    over (
  ...
  as es_reservado
     , 0 as es_nuevo, sum(distinct d.valor)
