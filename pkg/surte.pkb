...
--     function find_min(
--       p_calculo surte_calculo.header_nt
--     ) return number is
--       l_min number;
--     begin
--       l_min := p_calculo(1).detail(1).cant_final;
--       for i in 1 .. p_calculo.count loop
--         if p_calculo(1).detail(i).cant_final < l_min then
--           l_min := p_calculo(1).detail(i).cant_final;
--         end if;
--       end loop;
--       return l_min;
--     end;
...
--       p_calculo in out surte_calculo.header_nt
--     , p_cant_partir    number
--     ) is
--     begin
--       p_calculo(1).es_partible := true;
--       for i in 1 .. p_calculo(c_header).detail.count loop
--         if p_cant_partir * p_calculo(1).detail(i).rendimiento <= p_calculo(1).detail(i).cant_final then
--           p_calculo(1).detail(i).cant_final := p_cant_partir * p_calculo(1).detail(i).rendimiento;
--         else
...
--         p_juegos(p_idx).piezas(j).stock_actual := p_calculo(1).detail(j).stock_actual;
--         p_juegos(p_idx).piezas(j).saldo_stock := get_stock(l_codart) - p_calculo(1).detail(j).cant_final;
--         p_juegos(p_idx).piezas(j).cant_final := p_calculo(1).detail(j).cant_final;
--         p_juegos(p_idx).piezas(j).tiene_stock_itm :=
--             case when p_calculo(1).detail(j).tiene_stock then 1 else 0 end;
--         reduce_stock(l_codart, p_juegos(p_idx).piezas(j).cant_final);
--       end loop;
--     end;
    ...
    begin
      for i in 1 .. p_juegos.count loop
        for j in 1 .. p_juegos(i).piezas.count loop
          ...
          set_default(p_juegos(i), p_juegos(i).piezas(j), l_stock_actual);
